/**********************************************************
**                                                       **
**      $VER: Edit.c 1.1 (3.2.2001)                      **
**      Implementation of 'Edit' menu commands           **
...
pfnSelectFunc SwitchSelect( Project p, BYTE mode, BYTE force )
{
	...
	if(p->nbrc == p->ccp.xp && p->nbl == p->ccp.yp)
	{
		...
		{
			case 0:
				if( mode ) goto new_mode;
				RP->Mask = gui.selmask;  /* No break */
...
void line_selection( Project p )
{
	if( !p->ccp.select || p->ccp.yp == p->nbl )
	{
		click(p, p->xcurs, p->ycurs, TRUE);
		p->ccp.select = LINE_TYPE;
		(move_selection = move_stream_selection)(p, p->nbrc, p->nbl);
		inv_curs(p, TRUE);
	}
}
