...
Line create_line( STRPTR bytes, ULONG size )
{
	...
	Line           new   = AllocMem( sizeof(*new), MEMF_PUBLIC );

	/* Allocate struct & buf */
	if( new )
	{
		if(NULL != (new->stream = AllocMem(rsize, MEMF_PUBLIC)))
		{
...
/* Try to resize the memory chunk of text */
static ULONG realloc_stream( Line ln, ULONG size )
{
	/* Always adjust size to the granularity of mem */
	ULONG  rsize = (size & ~(GRANULARITY-1)) + GRANULARITY;
	STRPTR new   = AllocMem(rsize, MEMF_PUBLIC);
...
BOOL add_char( JBuf jb, Line ln, ULONG pos, UBYTE newchar )
{
	...
	else MemMove(ln->stream+pos,1,ln->size-pos), ln->size++;
...
BOOL insert_str( JBuf jb, Line ln, ULONG pos, STRPTR str, ULONG lg )
{
	...
	if( ln->size + lg > ln->max )
	{
		...
		else return FALSE;     /* Nothing change */
	}
	else /* Just shifts the buffer */
...
static BOOL break_line( JBuf jb, Line ln, ULONG pos, STRPTR str, ULONG lg )
{
	STRPTR oldbuf = ln->stream;
	ULONG  rsize  = ((pos+lg) & ~(GRANULARITY-1)) + GRANULARITY,
	       size   = ln->size-pos;
	Line   ln2    = create_line(oldbuf + pos, size);
...
BOOL replace_chars( Line ln, ULONG s, ULONG e, replace_func_t change )
{
	register STRPTR buf;
...
BOOL split_line( JBuf jb, Line ln, ULONG pos, ULONG *nbrwc, BOOL indent )
{
	Line new;
	LONG spc = 0;
