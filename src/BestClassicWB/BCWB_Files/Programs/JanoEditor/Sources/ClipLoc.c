...
static struct IFFHandle * clip = NULL;
...
STRPTR JanoMessages[] = {
	...
	ERR_ALREADYOPEN_STR,   ERR_SO_STR,
...
BOOL CBWriteFTXT( Line stsel, CCP * ccp )
{
	Line ln;
	long length;
...
BOOL CBReadCHRS( APTR jbuf, Line st, ULONG pos, LONG *nbl )
{
...
void InitLocale( void )
{
	if( (catalog = (APTR) OpenCatalogA(NULL, APPNAME ".catalog", NULL)) )
	{
		...
		{	register STRPTR * str;
			for(n = 0, str = JanoMessages; n < sizeof(JanoMessages)/sizeof(STRPTR); str++)
				if (*str != NULL)
					*str = (STRPTR) GetCatalogStr( catalog, n + ERR_BADOS, *str ), n++;
		}
	}
}
